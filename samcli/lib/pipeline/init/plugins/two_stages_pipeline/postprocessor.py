...
class Postprocessor(Processor):
    ...
    def run(self, context: Dict) -> Dict:
        ...
        if self.resources_created:
            ...
            for arn in self.resources_created:
                click.secho(f"\t{arn}", fg="yellow")
        ...
        if not deployer.is_user_provided:
            click.secho("Please set the following variables of the IAM user credentials to your CICD project:",
                        fg="green")
            click.secho(f"{plugin_context.deployer_aws_access_key_id_variable_name}: {deployer.access_key_id}",
                        fg="green")
            click.secho(f"{plugin_context.deployer_aws_secret_access_key_variable_name}: {deployer.secret_access_key}",
                        fg="green")

        return context

    def _categorize_resource(self, resource: Resource, required_permissions: str):
        """
        ...
        if resource.is_user_provided:
            ...
            self.resources_created.append(resource.arn)
