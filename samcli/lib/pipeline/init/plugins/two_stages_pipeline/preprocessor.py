...
class Preprocessor(Processor):
    ...
    def run(self, context: Dict) -> Dict:
        ...
        deployer = plugin_context.deployer
        if deployer.is_user_provided:
            deployer_arn = deployer.arn
        else:
            # create deployer(IAM user) in the testing stage
            testing_stage = plugin_context.get_stage(PluginContext.TESTING_STAGE_NAME)
        ...
        for stage in plugin_context.stages:
            deployer_role_arn, cfn_deployment_role_arn, artifacts_bucket_arn, kms_key_arn =\
    ...
    def _get_build_image(sam_template_file: str) -> str:
        ...
        if not runtimes:
            return "public.ecr.aws/sam/build-provided"
        elif len(runtimes) > 1:
            click.echo("The SAM template defines multiple functions with different runtimes\n"
            ...
            if not build_image:
                click.echo(f"The SAM template defines functions of runtime {runtime} but SAM doesn't have a docker "
                           f"build-image for {runtime}, please provide one")
                build_image = click.prompt("Docker Build image")
            return build_image

    @staticmethod
    def _create_deployer_at(stage: Stage):
        """
        ...
        stack_name = f"{STACK_NAME_PREFIX}-{stage.stack_name}-{DEPLOYER_STACK_NAME_SUFFIX}"
        deployer_template_path = os.path.join(CFN_TEMPLATE_PATH, "deployer.yaml")
        ...
        deployer = next(o for o in outputs if o.get("OutputKey") == "Deployer")
        access_key_id = next(o for o in outputs if o.get("OutputKey") == "AccessKeyId")
        secret_access_key = next(o for o in outputs if o.get("OutputKey") == "SecretAccessKey")
        return deployer.get("OutputValue"), access_key_id.get("OutputValue"), secret_access_key.get("OutputValue")

    @staticmethod
    def _create_missing_stage_resources(stage: Stage, deployer_arn: str):
        """
        ...
        if stage.did_user_provide_all_required_resources():
            ...
            return stage.deployer_role.arn, stage.cfn_deployment_role.arn, stage.artifacts_bucket.arn, stage.artifacts_bucket.kms_key_arn
        missing_resources = ""
        ...
        stage_resources_template_path = os.path.join(CFN_TEMPLATE_PATH, "stage_resources.yaml")
        stack_name = f"{STACK_NAME_PREFIX}-{stage.stack_name}-{STAGE_RESOURCES_STACK_NAME_SUFFIX}"
        with open(stage_resources_template_path, "r") as fp:
            stage_resources_template_body = fp.read()
        output = manage_cloudformation_stack(
        ...
        deployer_role_arn = next(o for o in output if o.get("OutputKey") == "DeployerRole").get("OutputValue")
        cfn_deployment_role_arn = next(o for o in output if o.get("OutputKey") == "CFNDeploymentRole").get("OutputValue")
        artifacts_bucket_arn = next(o for o in output if o.get("OutputKey") == "ArtifactsBucket").get("OutputValue")
        try:
            artifacts_bucket_key_arn = next(o for o in output if o.get("OutputKey") == "ArtifactsBucketKey").get("OutputValue")
